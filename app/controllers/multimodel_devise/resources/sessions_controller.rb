class MultimodelDevise::Resources::SessionsController < Devise::SessionsController
  ...
  def create
    ...
    if @resource.present? && @resource.valid_password?(session_params[:password])
      ...
      render json: @resource, root: 'user', serializer: MultimodelDevise::SessionSerializer, status: :ok
    else
  ...
  def authentication_error(status)
    MultimodelDevise::ErrorReport.generate({authentication: [t(".auth_fail")]}, status)
  end
